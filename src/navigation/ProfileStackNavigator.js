...
export const ProfileStackNavigator = ({user, theme, isDarkMode}) => (
    <Stack.Navigator
        ...
        <Stack.Screen name="Profile" options={{title: 'Профиль', headerShown: false}} >{(props) =>
            <Profile {...props} user={user} theme={theme} isDarkMode={isDarkMode}/>}</Stack.Screen>
        <Stack.Screen name="Auth" options={{title: 'Вход и регистрация', headerShown: false}}>{(props) =>
            <Auth {...props} user={user} theme={theme} isDarkMode={isDarkMode}/>}</Stack.Screen>
    </Stack.Navigator>
);